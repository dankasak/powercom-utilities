...
sub fetch_database_list {
    ...
    carp( "Can't call Database::Connection::SQLite::fetch_database_list() ... SQLite doesn't have database containers" );
...
sub create_model_schema {
    ...
    $self->do(
        "create table target_db_schemas (\n"
    ...
    $self->do(
        "create table if not exists view_manglers (\n"
      . "    ID                    integer       primary key\n"
    ...
    $self->{_model_index_insert_sth} = $self->prepare(
        "insert into indexes\n"
      . "(\n"
      . "    database_name , schema_name , table_name , index_name , is_primary , is_unique, is_distribution_key\n"
      . ") values ( ? , ? , ? , ? , ? , ?, ? )"
    );
    ...
    $self->{_model_view_insert_sth} = $self->prepare(
        "insert into views\n"
      . "(\n"
      . "    database_name , schema_name , view_name , view_definition\n"
      . ") values ( ? , ? , ? , ? )"
    );
