...
sub dialog {
    ...
    if ( $options->{type} eq 'options' ) {
        ...
        my $font     = Pango::FontDescription::from_string( 'FreeMono 10' );
...
sub load_window {
    ...
    my @all_paths = $globals->{db}->all_gui_paths;
...
sub get_icon_path {
    ...
    foreach my $include_path ( @all_paths ) {
        if ( -e $include_path . "/icons/" . $icon_name ) {
            print "Loading icon resource [" . $include_path . "/icons/" . $icon_name . "\n";
            return $include_path . "/icons/" . $icon_name;
        }
    }
...
sub get_db_icon_path {
    ...
    foreach my $include_path ( @all_paths ) {
        if ( -e $include_path . "/icons/dbs/" . $icon_name ) {
            print "Loading db icon resource [" . $include_path . "/icons/dbs/" . $icon_name . "\n";
            return $include_path . "/icons/dbs/" . $icon_name;
        }
    }
    ...
    if ( $header_bar ) {
        ...
        foreach my $gui_overlay_paths ( $self->{globals}->{local_db}->gui_overlay_paths ) {
            ...
            if ( -e $this_overlay_path ) {
                ...
                warn "Overlay path [$this_overlay_path] didn't contain a menu_control.json file!";
            }
        }
        ...
        foreach my $menu_control_path ( @menu_controls ) {
            ...
            foreach my $menu_item ( @{ $menu_control->{items} } ) {
                ...
                my $this_menu_image = Gtk3::Image->new_from_icon_name( $menu_item->{image}, 'GTK_ICON_SIZE_MENU' ); # GTK_ICON_SIZE_LARGE_TOOLBA 
...
sub open_pdf {
    ...
    if ( $^O eq "linux" ) {
        system( $self->{globals}->{apps}->{pdf_viewer} . " $pdf_path &" );
    } else {
        system( "start " . $self->{globals}->{apps}->{pdf_viewer} . " $pdf_path" );
    }
...
sub edit {
    ...
    if ( $^O eq "linux" ) {
        system( $self->{globals}->{apps}->{editor} ." $file &" );
    } else {
        system( "start " . $self->{globals}->{apps}->{editor} . " $file" );
    }
...
sub to_pixbuf {
    ...
    return Gtk3::Image->new_from_file( $file )->get_pixbuf;
...
sub kick_gtk {
    ...
    if ( $ENV{GDK_BACKEND} eq 'wayland' ) {
        warn "Not kicking gtk under wayland";
        return;
    } elsif ( $self->{globals}->{self}->{broadway} ) {
        for my $i ( 1 .. 10 ) {
            Gtk3::main_iteration();
        }
    } else {
